...
public class GraphBuilderCache {
    ...
    protected Node getRouteStation(Transaction txn, RouteReadOnly route, IdFor<Station> stationId) {
    ...
    protected void putService(IdFor<RouteReadOnly> routeId, Service service, IdFor<Station> begin, IdFor<Station> end, Node svcNode) {
    ...
    protected Node getServiceNode(Transaction txn, IdFor<RouteReadOnly> routeId, Service service, IdFor<Station> startStation, IdFor<Station> endStation) {
        String id = CreateKeys.getServiceKey(routeId, service.getId(), startStation, endStation);
        return txn.getNodeById(svcNodes.get(id));
    }

    protected void putHour(IdFor<RouteReadOnly> routeId, Service service, IdFor<Station> station, Integer hour, Node node) {
        hourNodes.put(CreateKeys.getHourKey(routeId, service.getId(), station, hour), node.getId());
    }

    protected Node getHourNode(Transaction txn, IdFor<RouteReadOnly> routeId, Service service, IdFor<Station> station, Integer hour) {
    ...
    public boolean hasServiceNode(IdFor<RouteReadOnly> routeId, Service service, IdFor<Station> begin, IdFor<Station> end) {
        return svcNodes.containsKey(CreateKeys.getServiceKey(routeId, service.getId(), begin,end));
    }

    public boolean hasHourNode(IdFor<RouteReadOnly> routeId, Service service, IdFor<Station> startId, Integer hour) {
        return hourNodes.containsKey(CreateKeys.getHourKey(routeId, service.getId(), startId, hour));
    }

    private static class CreateKeys {

        protected static String getServiceKey(IdFor<RouteReadOnly> routeId, IdFor<Service> service,
        ...
        @Deprecated
        protected static String getHourKey(IdFor<RouteReadOnly> routeId, IdFor<Service> service, IdFor<Station> station, Integer hour) {
